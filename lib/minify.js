...
const minifyJs = async() => {
    const jsFiles = await glob([
        `${path.join(process.cwd(), config.buildDir)}/**/*.js`,
        `!${path.join(process.cwd(), config.buildDir)}/**/*.min.js`
    ]);
...
const minifyCss = async() => {
    const cssFiles = await glob([
        `${path.join(process.cwd(), config.buildDir)}/**/*.css`,
        `!${path.join(process.cwd(), config.buildDir)}/**/*.min.css`
    ]);
