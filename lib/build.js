...
const { setupHelpers, compile, render, globPath, registerPartials } = require('./common');
...
const buildIndex = async () => {
    ...
    try{
        ...
        // Check for swagger and add if enabled
        if(process.config.swaggerDocs && process.config.swaggerDocs.enabled){
            pageData.swagger = await getSwagger();
        }
...
const getSwagger = async () => {
    // Swagger file path
    const swaggerPath = path.join(process.config.sourceDir, process.config.swaggerDocs.swaggerFile);

    // File checks
    if(!fs.existsSync(swaggerPath)){
        console.log(`${chalk.red('Required file not found')}: ${swaggerPath}`);
        process.exit();
    }

    // Read in swagger file
    const swaggerFile = fs.readFileSync(swaggerPath, 'utf-8');

    // Check for JSON and parse
    if(process.config.swaggerDocs.type && process.config.swaggerDocs.type === 'json'){
        try{
            return JSON.parse(swaggerFile);
        }catch(ex){
            console.log(`${chalk.red('Failed to parse JSON formatted Swagger file')}: ${swaggerPath}`);
            process.exit();
        }
    };

    // Default to YAML swagger
    try{
        const swaggerObj = yaml.load(swaggerFile);
        return swaggerObj;
    }catch(ex){
        console.log(`${chalk.red('Failed to parse YAML formatted Swagger file')}: ${swaggerPath}`);
        process.exit();
    }
};
...
const rootFiles = async () => {
    try{
        ...
        const rootFiles = await glob([
            `${globPath(process.config.sourceDir)}/*`,
            `!${globPath(process.config.sourceDir)}/*.${process.config.templateEngine}`
        ]);
