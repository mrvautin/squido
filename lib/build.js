...
const fs = require('fs');
const { getMeta, getConfig, setupHelpers, compile, render } = require('./common');
...
const buildIndex = async () => {
    ...
    // Template setup
    const layout = compile(fs.readFileSync(layoutPath, 'utf-8'));
...
const rootFiles = async () => {
    try{
        ...
        const rootFiles = await glob([
            `${config.sourceDir}/*`,
            `!${config.sourceDir}/*.${config.templateEngine}`
        ]);
