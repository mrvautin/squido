...
const buildIndex = async () => {
    ...
    if(!fs.existsSync(layoutPath)){
        console.log(`${chalk.red('Requied file not found')}: ${layoutPath}`);
        process.exit();
    }
    if(!fs.existsSync(templateIndexPath)){
        console.log(`${chalk.red('Requied file not found')}: ${templateIndexPath}`);
        process.exit();
    }
    if(!fs.existsSync(template404Path)){
        console.log(`${chalk.red('Requied file not found')}: ${template404Path}`);
        process.exit();
    }
