...
const buildPost = async (file) => {
    ...
    const layoutPath = path.join(config.sourceDir, 'layouts', 'layout.hbs');
    const templatePath = path.join(config.sourceDir, 'post.hbs');
    ...
    try{
        ...
        if(meta){
            ...
            if(!fs.existsSync(path.join(config.buildDir, meta.permalink))){
                fs.mkdirSync(path.join(config.buildDir, meta.permalink));
            }

            // Render html
            const postHtml = template(Object.assign(meta, { postList: process.postList }));
