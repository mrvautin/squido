...
const getMeta = async (filepath) => {
    ...
    try{
        await matterSchema.validate(frontMatter);
    }catch(ex){
        console.log(chalk.red('Error(s) parsing the following file: '), chalk.white(frontMatter.file));
        console.log(chalk.red('Errors:'), chalk.yellow(ex.errors));

        // Add some more help for Date formatting
        if(ex.path === 'date'){
            const todaysDate = new Date();
            console.log(chalk.red('Supported format examples:'));
            console.log(chalk.yellow(todaysDate.toDateString()));
            console.log(chalk.yellow(todaysDate.toLocaleDateString()));
            console.log(chalk.yellow(todaysDate.toUTCString()));
            console.log(chalk.yellow(todaysDate.toLocaleString()));
        }
        process.exit();
    }
    ...
    if(!meta.description || meta.description === ''){
        meta.description = trimSummary(meta.summary, 160);
    }
...
const registerPartials = async (data) => {
