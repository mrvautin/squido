...
const getMeta = (file) => {
    const fileId = md5File.sync(file);
    ...
    const bodyText = sourceFile.replace(bodyRegex, '');
    meta.body = markdown.render(bodyText);
    meta.markdown = bodyText;
    meta.filename = file;
    meta.id = fileId;
    meta.fileMeta = yaml.load(metaMatch[0]);
    meta.summary = stripHtml(meta.body).result.substring(0, config.summaryLength || 250);
    meta.date = new Date(meta.date);
    meta.dateISO = new Date(meta.date).toISOString();
    meta.lastupdated = fileStats.mtime;
...
const runSetupNew = async () => {
    ...
    const setupFiles = await glob([
        `${path.join(__dirname, '..', 'source')}/**/*.hbs`,
        `${path.join(__dirname, '..', 'source')}/package.json`,
        `${path.join(__dirname, '..', 'source')}/robots.txt`,
        `${path.join(__dirname, '..', 'source')}/content/**/*`,
        `${path.join(__dirname, '..', 'source')}/posts/**/*.markdown`,
        `!${path.join(__dirname, '..', 'source')}/post-diff.hbs`
    ]);
